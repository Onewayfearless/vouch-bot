...
// Applies a server's saved watching status to the bot presence.
// Note: Discord only supports ONE global bot avatar/banner per account, so true
// per-server pfp/banner isn't possible at the account level. We store each
// server's choice and show it on that server's embeds (footer/thumbnail) instead.
function applyServerIdentity(guildId) {
  try {
    const g = getGuild(guildId);
    if (g.config.server_watching) {
      client.user.setActivity(g.config.server_watching, { type: ActivityType.Watching });
    }
  } catch (e) { /* ignore */ }
}
...
Use \`$setbotowner @user\` to transfer ownership.`);
  ...
  if (command === 'revamp') {
    const menu = new StringSelectMenuBuilder()
      ...
      .addOptions(
        new StringSelectMenuOptionBuilder().setLabel('Embed Color').setValue('embed_color').setDescription('Hex color e.g. #FF0000'),
        new StringSelectMenuOptionBuilder().setLabel('Embed Footer').setValue('embed_footer').setDescription('Footer text on vouch embeds'),
        new StringSelectMenuOptionBuilder().setLabel('Embed Banner/Image').setValue('embed_image').setDescription('Image URL shown in vouch embeds'),
        new StringSelectMenuOptionBuilder().setLabel('Bot Avatar (this server)').setValue('bot_avatar').setDescription('Change bot pfp for this server (image URL)'),
        new StringSelectMenuOptionBuilder().setLabel('Bot Banner (this server)').setValue('bot_banner').setDescription('Change bot banner for this server'),
        new StringSelectMenuOptionBuilder().setLabel('Bot Bio').setValue('bot_bio').setDescription('Change bot about me'),
        new StringSelectMenuOptionBuilder().setLabel('Bot Watching (this server)').setValue('bot_watching').setDescription('Set watching status for this server'),
      );
    ...
    collector.on('collect', async (interaction) => {
      ...
      valueCollector.on('collect', async (valueMsg) => {
        ...
        try {
          ...
          if (choice === 'embed_image')   { g.config.embed_image  = value; await save(); return valueMsg.reply(`embed banner set for this server`); }
          if (choice === 'bot_avatar')    { g.config.server_avatar = value; await save(); return valueMsg.reply(`bot avatar saved for **${guild.name}** (shows on this server's vouch embeds)`); }
          if (choice === 'bot_banner')    { g.config.server_banner = value; await save(); return valueMsg.reply(`bot banner saved for **${guild.name}**`); }
          if (choice === 'bot_bio')       { g.config.server_bio = value; await save(); return valueMsg.reply(`bot bio saved for **${guild.name}**`); }
          if (choice === 'bot_watching')  { g.config.server_watching = value; await save(); applyServerIdentity(guildId); return valueMsg.reply(`watching status set to **${value}** for this server`); }
        } catch (err) {
  ...
  if (command === 'setwatching') {
    ...
    return message.reply(`bot's now watching **${text}**`);
  }

  } catch (err) {
    console.error(`[Command error] $${command} in guild ${guildId}:`, err);
    try { await message.reply('somethin broke runnin that, try again in a sec'); } catch {}
  }
});
