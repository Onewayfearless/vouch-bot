...
function buildVouchEmbed(guildId, targetUser, reviewerUser, total, reviewText) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: `Reviews (showing ${shown.length}${all.length > 20 ? ' of ' + all.length : ''})`, value: recentLines, inline: false }
    )
...
function buildAllReviewsEmbeds(guildId, targetUser) {
  ...
  const perPage = 25;
  const embeds = [];
  for (let p = 0; p < all.length && embeds.length < 10; p += perPage) {
...
function startAutoTimer(guildId, guild) {
  ...
  autoTimers[guildId] = setInterval(async () => {
    if (autoBusy[guildId]) return;
    autoBusy[guildId] = true;
    try {
      const g = getGuild(guildId);
      ...
      if (!g.config.auto_queue || g.config.auto_queue.length === 0) {
        ...
        const ids = guild.roles.cache.get(g.config.auto_target_role)?.members.filter(m => !m.user.bot).map(m => m.id) ?? [];
        if (!ids.length) { console.warn(`[Auto ${guildId}] target role empty`); return; }
        g.config.auto_queue = ids;
      }
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    if (!embeds.length) return interaction.reply({ content: 'nothin to show', ephemeral: true });
    await interaction.reply({ embeds, ephemeral: true });
  } catch (err) {
    console.error('[ShowMore]', err.message);
    if (!interaction.replied) interaction.reply({ content: 'somethin broke try again', ephemeral: true }).catch(() => {});
  }
});
...
client.on('messageCreate', async (message) => {
  ...
  try {
    ...
    if (command === 'profile') {
      ...
      const shown = all.slice(0, 20);
      const config = getGuild(guildId).config;
      const lines = shown.length ? shown.map((r, i) => {
        const ts = Math.floor(r.created_at / 1000);
        return `\`${String(i + 1).padStart(2, '0')}\` ${r.review_text} — <@${r.reviewer_user_id}> <t:${ts}:d>`;
      }).join('\n') : 'No reviews yet';
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Rank', value: rank.name, inline: true },
          { name: 'Progress', value: progressBar(total, rank.nextAt), inline: true },
          { name: `Reviews (showing ${shown.length}${all.length > 20 ? ' of ' + all.length : ''})`, value: lines, inline: false },
    ...
    if (command === 'autoconfig' || command === 'help') {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Public', value: '`$profile [@user]`\n`$leaderboard`\n`$recentreviews [@user]`\n`$admins`' }
        )
